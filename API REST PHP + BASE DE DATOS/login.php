...
switch ($method) {
   case 'GET':
		...
		if ($_SERVER['REQUEST_URI'] === '/api/login.php/users') {
			...
			$query = "SELECT id, email, password FROM users order by id desc";
			$stmt = $db->query($query);
    ...
    case 'POST':
        ...
        if ($_SERVER['REQUEST_URI'] === '/api/login.php/login') {
            ...
            if (!$user) {
                ...
                echo json_encode(['success' => false, 'message' => 'Credenciales inválidas - Usuario no encontrado']);
            } else if (!password_verify($password, $user['password'])) {
                // Contraseña incorrecta
                header('HTTP/1.1 401 Unauthorized');
                $passwordDB = $user['password'];  
                echo json_encode(['success' => false, 'message' => 'Credenciales inválidas -Contraseña incorrecta', 'password' => $password, 'email' => $email,  'passwordDB' => $passwordDB  ]);                
            } else {
            ...
            if ($existingUser) {
                ...
                echo json_encode(['success' => false, 'message' => 'El usuario ya existe, por favor revisar']);              
            } else {
